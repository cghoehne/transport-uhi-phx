...
uza.weather <- readRDS(here("data/2017-uza-weather-data.rds"))

# uza station data (spatial points w/ metadata)
uza.stations <- readRDS(here("data/uza-station-data.rds"))
...
#uza.roads <- readRDS(here("data/osm-links-buffers-sp-list.rds"))
...
# save osm roadway data to examine in QGIS for errors (TEMPORARY)
for(i in 1:length(uza.roads)){
  shapefile(uza.roads[[i]], here(paste0("data/shapefiles/osm/osm_buffered_",i)))
}
