...
# import runs for thermal variying thermal inertia
all.surface.data <- readRDS(here("data/outputs/run_metadata_20190526_182759_varied_albedo/all_pave_surface_data.rds"))
all.model.runs <- readRDS(here("data/outputs/run_metadata_20190526_182759_varied_albedo/stats_all_model_runs.rds"))

# calc the thermal inertia values for layers
all.model.runs[, L1.a := sqrt(L1.k * L1.rho * L1.c)]   # UNITS: (W/(m*degK)) * (kg/m3) * (J/(kg*degK) = J/(m2*degK*(s^0.5))
all.model.runs[, L2.a := sqrt(L2.k * L2.rho * L2.c)]   # UNITS: (W/(m*degK)) * (kg/m3) * (J/(kg*degK) = J/(m2*degK*(s^0.5))
#all.model.runs[, L3.TI := sqrt(L3.k * L3.rho * L3.c)]   # UNITS: (W/(m*degK)) * (kg/m3) * (J/(kg*degK) = J/(m2*degK*(s^0.5))
#all.model.runs[, L4.TI := sqrt(L4.k * L4.rho * L4.c)]   # UNITS: (W/(m*degK)) * (kg/m3) * (J/(kg*degK) = J/(m2*degK*(s^0.5))
