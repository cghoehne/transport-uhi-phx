...
lhs.dig <- function(x) ifelse(abs(x) < 1, 0, nchar(as.character(as.integer(x))))
...
t.agg <-
  ...
  summarise(yr_max_temp = max(temp.f, na.rm = T),
            yr_avg_temp = mean(temp.f, na.rm = T),
            smr_min_temp = min(temp.f[month %in% c("Jun","Jul","Aug")], na.rm = T),
            smr_5pm_temp = mean(temp.f[hour == 17], na.rm = T),
            smr_7pm_temp = mean(temp.f[hour == 19], na.rm = T),
            smr_9pm_temp = mean(temp.f[hour == 21], na.rm = T)) %>%
  data.table() 
...
for(variable.j in variables){ # each summary variable j
  for(buffer.i in 1:length(uza.stations)){ # each buffer index i
  
    # models
    models[[buffer.i]] <- lm(unlist(uza.stations[[buffer.i]][,mget(variable.j)]) ~ uza.stations[[buffer.i]]$pave.prct)
    ...
    # plots
    range.x <- diff(range(uza.stations[[buffer.i]]$pave.prct))
    range.y <- diff(range(unlist(uza.stations[[buffer.i]][,mget(variable.j)]), na.rm = T))
    min.x <- 0
    max.x <- 1
    min.y <- min()
    max.y <- 
    ...
    models.pl[[buffer.i]] <- (ggplot()
          ...
          + labs(x = paste0("Fractional Surrounding Area of Road & Parking Pavement (r=",radii.buffers[[buffer.i]],"ft)"), y = "2017 Max Observed Temperature")
          + geom_text(aes(x = 0.75, y = 108), label = paste("R^2 ==",signif(summary(models[[buffer.i]])$r.squared, 2)), parse = T, family = my.font)
          + geom_text(aes(x = 0.75, y = 105), label = paste("p ==",signif(anova(models[[buffer.i]])$'Pr(>F)'[1], 2)), parse = T, family = my.font)
          + geom_text(aes(x = 0.75, y = 102), label = paste("n ==",nrow(models.dt[[buffer.i]])), parse = T, family = my.font)
          + theme_minimal()
          + theme(text = element_text(family = my.font, size = 12),
                  plot.margin = margin(t = 10, r = 20, b = 10, l = 10, unit = "pt")))
    
    # save plots as png
    var.path <- here(paste0("figures/",variable.j)) # string of new variable path sub-directory
