...
all.data
all.data <- lapply(1:length(all.data), function(x) all.data[[x]][, pave.prct := road.prct + park.prct]) # create pavement total %
w.data.agg <- all.data[[2]][source != "iButton", .(max.temp = max(heat.f, na.rm = T), road.prct = mean(road.prct, na.rm = T), park.prct = mean(park.prct, na.rm = T)), by = .(station.name)] # aggregate
...
lm1 <- lm(w.data.agg[pave.prct != 0]$max.temp ~ w.data.agg[pave.prct != 0]$pave.prct)
summary(lm1)
(ggplot(aes(y = max.temp, x = pave.prct), data = w.data.agg[pave.prct != 0]) 
  + geom_point()
  + stat_smooth(method = "lm")
  + geom_text(aes(x = 0.1, y = 117), label = paste("R^2 ==",round(summary(lm1)$r.squared, 2)), parse = T)
  )

# heat change relative to hourly mean (May through September)
w.data.agg2 <- all.data[[3]][source != "iButton"]
w.data.agg2 <- w.data.agg2[, hour.mean := mean(heat.f, na.rm = T), by = .(hour,station.name)]
w.data.agg2[, heat.chng := heat.f - hour.mean]

lm2 <- lm(w.data.agg2[pave.prct != 0]$heat.chng ~ w.data.agg2[pave.prct != 0]$pave.prct)
summary(lm2)
